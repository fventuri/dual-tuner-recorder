...
static const sdrplay_api_AgcControlT agc_enabled_default = sdrplay_api_AGC_50HZ;
...
static const char *serial_number = NULL;
static double rspduo_sample_rate = 0.0;
static int decimation = 1;
static sdrplay_api_If_kHzT if_frequency = sdrplay_api_IF_Zero;
static sdrplay_api_Bw_MHzT if_bandwidth = sdrplay_api_BW_0_200;
static sdrplay_api_AgcControlT agc_A = sdrplay_api_AGC_DISABLE;
static sdrplay_api_AgcControlT agc_B = sdrplay_api_AGC_DISABLE;
static int gRdB_A = 40;
static int gRdB_B = 40;
static int LNAstate_A = 0;
static int LNAstate_B = 0;
static int DCenable = 1;
static int IQenable = 1;
static int dcCal = 3;
static int speedUp = 0;
static int trackTime = 1;
static int refreshRateTime = 2048;
static double frequency_A = 100e6;
static double frequency_B = 100e6;
static int streaming_time = 10;  /* streaming time in seconds */
static int marker_interval = 0;  /* store a marker tick every N seconds */
static char *outfile_template = NULL;
static OutputType output_type = OUTPUT_TYPE_RAW;
static unsigned int zero_sample_gaps_max_size = 100000;
#ifndef WIN32
static unsigned int blocks_buffer_capacity = 2000;
static unsigned int samples_buffer_capacity = 1048576;
#else
/* Windows requires larger buffers */
static unsigned int blocks_buffer_capacity = 16000;
static unsigned int samples_buffer_capacity = 8388608;
#endif
static int gain_changes_buffer_capacity = 100;
static int gains_file_enable = 0;
...
{
    int c;
    while ((c = getopt(argc, argv, "c:s:r:d:i:b:g:l:DIy:f:x:m:o:z:j:k:RLWGXvh")) != -1) {
        int n;
        switch (c) {
            case 'c':
            ...
            case 'g':
                if (strcmp(optarg, "AGC") == 0 || strcmp(optarg, "AGC,AGC") == 0) {
                    agc_A = agc_enabled_default;
                    agc_B = agc_enabled_default;
...
static int read_config_grdb(const char *valuestr, sdrplay_api_AgcControlT *agc_first, sdrplay_api_AgcControlT *agc_second, int *gRdB_first, int *gRdB_second) {
    ...
    if (nn == 1) {
        if (strcasecmp(first, "AGC") == 0) {
            *agc_first = agc_enabled_default;
            *agc_second = agc_enabled_default;
        } else {
            int tmp;
            int n;
        ...
        int tmp_gRdB_second;
        if (strcasecmp(first, "AGC") == 0) {
            tmp_agc_first = agc_enabled_default;
        } else {
            int n;
        ...
        if (strcasecmp(second, "AGC") == 0) {
            tmp_agc_second = agc_enabled_default;
        } else {
            int n;
...
static int read_config_file(const char *config_file) {
    ...
    while (read_config_file_iter(&key, &value) != EOF) {
        ...
        } else if (strcasecmp(key, "rspduo sample rate") == 0) {
            read_config_status = read_config_double(value, &rspduo_sample_rate);
